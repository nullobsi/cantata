...
The ``FFMPEG_ROOT`` variable can be set to influence where this module
will search for the libraries.
...
For each requested component, the imported target ``FFMPEG::<component>`` is created.
...
``FFMPEG_FOUND``
  True if the system has the FFMPEG libraries.
``FFMPEG_VERSION``
  The version of FFMPEG which was found.
``FFMPEG_INCLUDE_DIRS``
  Include directories needed to use FFMPEG.
``FFMPEG_LIBRARIES``
  Libraries needed to link to FFMPEG. This only included libraries for the requested components.

For each component requested, the following variables are provided:

``FFMPEG_<component>_FOUND``
  True if the system has the requested component.
``FFMPEG_<component>_INCLUDE_DIRS``
  Include directories needed to use the component.
``FFMPEG_<component>_LIBRARIES``
  Libraries needed to link to the component.
``FFMPEG_<component>_VERSION``
  The version of the component which was found.
...
The following cache variables may also be set for each component:

``FFMPEG_<component>_LIBRARY``
  The path to the component library.
``FFMPEG_<component>_INCLUDE_DIR``
  The directory containing the component header.
#]==]
