...
#define VALIDATE_MAX_WINDOW \
	((3ul << 30) / VALIDATE_MAX_SAMPLERATE / VALIDATE_MAX_CHANNELS / sizeof(double))
...
ebur128_init(unsigned int channels, unsigned long samplerate, int mode)
{
	...
	if (st->d->audio_data_frames % st->d->samples_in_100ms) {
		...
		st->d->audio_data_frames =
				(st->d->audio_data_frames + st->d->samples_in_100ms) - (st->d->audio_data_frames % st->d->samples_in_100ms);
	}
	st->d->audio_data = (double*)malloc(st->d->audio_data_frames * st->channels * sizeof(double));
...
#define EBUR128_FILTER(type, min_scale, max_scale)                                                                         \
	...
	{                                                                                                                      \
		...
		if ((st->mode & EBUR128_MODE_TRUE_PEAK) == EBUR128_MODE_TRUE_PEAK && st->d->interp) {                              \
			for (i = 0; i < frames; ++i) {                                                                                 \
				for (c = 0; c < st->channels; ++c) {                                                                       \
		...
		for (c = 0; c < st->channels; ++c) {                                                                               \
			...
			for (i = 0; i < frames; ++i) {                                                                                 \
				st->d->v[c][0] =                                                                                           \
						(double)((double)src[i * st->channels + c] / scaling_factor) - st->d->a[1] * st->d->v[c][1] - /**/ \
						st->d->a[2] * st->d->v[c][2] -                                                                /**/ \
						st->d->a[3] * st->d->v[c][3] -                                                                /**/ \
						st->d->a[4] * st->d->v[c][4];                                                                      \
	...
	for (c = 0; c < st->channels; ++c) {
		...
		if (st->d->audio_data_index < frames_per_block * st->channels) {
			for (i = 0; i < st->d->audio_data_index / st->channels; ++i) {
				channel_sum += st->d->audio_data[i * st->channels + c] * st->d->audio_data[i * st->channels + c];
			}
			for (i = st->d->audio_data_frames - (frames_per_block - st->d->audio_data_index / st->channels);
		...
		if (st->d->channel_map[c] == EBUR128_Mp110 || st->d->channel_map[c] == EBUR128_Mm110 || st->d->channel_map[c] == EBUR128_Mp060 || st->d->channel_map[c] == EBUR128_Mm060 || st->d->channel_map[c] == EBUR128_Mp090 || st->d->channel_map[c] == EBUR128_Mm090) {
			channel_sum *= 1.41;
		}
			...
			if (frames >= st->d->needed_frames) {                                                                                         \
				...
				if ((st->mode & EBUR128_MODE_LRA) == EBUR128_MODE_LRA) {                                                                  \
					...
					if (st->d->short_term_frame_counter == st->d->samples_in_100ms * 30) {                                                \
						struct ebur128_dq_entry* block;                                                                                   \
						double st_energy;                                                                                                 \
						if (ebur128_energy_shortterm(st, &st_energy) == EBUR128_SUCCESS && st_energy >= histogram_energy_boundaries[0]) { \
							if (st->d->use_histogram) {                                                                                   \
								++st->d->short_term_block_energy_histogram                                                                \
